...
class Variation_model extends CI_Model
{
    ...
    public function get_variation_options($variation_id, $stock = null)
    {
        $stock_sql = '';
        if ($stock = clean_number($stock)) {
          $stock_sql = "AND stock >= $stock";
        }
        $sql = "SELECT * FROM variation_options WHERE variation_id = ? $stock_sql ORDER BY id";
    ...
    {
        $this->session->set_userdata('mc20bt99_vr_images_ary', $images);
    }
    ...
    {
        $images = array();
        if (!empty($this->session->userdata('mc20bt99_vr_images_ary'))) {
            $images = $this->session->userdata('mc20bt99_vr_images_ary');
        }
        if (!empty($images)) {
    ...
    {
        if (!empty($this->session->userdata('mc20bt99_vr_images_ary'))) {
            $this->session->unset_userdata('mc20bt99_vr_images_ary');
        }
    }
