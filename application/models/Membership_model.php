...
class Membership_model extends CI_Model
{
    ...
    public function add_membership_transaction($data_transaction, $plan)
    {
        ...
        if ($this->db->insert('membership_transactions', $data)) {
            $this->session->set_userdata('mc20bt99_membership_transaction_insert_id', $this->db->insert_id());
            if (!is_vendor()) {
                $data_user = array(
    ...
    */
	...
	public function edit_role($id)
	{
		...
		if (!empty($role)) {
			...
			return $this->db->where('id', $role->id)->update('roles_permissions', $data);
		}
		return false;
	}
	...
	//close user shop
	public function close_user_shop($user_id)
	{
		$user = get_user($user_id);
		if (!empty($user)) {
			return $this->change_user_role($user->id, 3);
		}
		return false;
	}
		...
		if (!empty($role)) {
			...
			if (!empty($users)) {
				foreach ($users as $user) {
					$this->db->where('id', $user->id)->update('users', ['role_id' => 3]);
				}
			}
