...
class Auth_model extends CI_Model
{
    ...
    {
        $data = array(
            'username' => remove_special_characters(str_slug($this->input->post('first_name', true).'-'.$this->input->post('last_name', true))),
    ...
    public function login()
    {
        ...
        if (!empty($user)) {
            ...
            $user_data = array(
                ...
                'modesy_sess_user_role' => $user->role_id,
    ...
    public function login_with_facebook($fb_user)
    {
        if (!empty($fb_user)) {
            ...
            // add queue
            $queue_id = add_queue('user_id', $user->id);
        }
    }
    ...
    public function register()
    {
        ...
        */
        if ($this->db->insert('users', $data)) {
            ...
            // add queue
            $queue_id = add_queue('user_id', $last_id);
            return $last_id;
        } else {
            return false;
        }
	...
	public function get_user_query()
	{
		return $this->db->select(
			'users.*, (SELECT permissions FROM roles_permissions WHERE roles_permissions.id = users.role_id LIMIT 1) AS permissions'
		);
	}
    ...
    {
		return $this->get_user_query()->where('users.id', clean_number($id))->get('users')->row();
    }

    //get user by email
    public function get_user_by_email($email)
    {
		return $this->get_user_query()->where('users.email', remove_special_characters($email))->get('users')->row();
    }

    //get user by username
    public function get_user_by_username($username)
    {
		return $this->get_user_query()->where('users.username', remove_special_characters($username))->get('users')->row();
    }

    //get user by shop name
    public function get_user_by_shop_name($shop_name)
    {
		return $this->get_user_query()->where('users.shop_name', remove_special_characters($shop_name))->get('users')->row();
    }

    //get user by slug
    public function get_user_by_slug($slug)
    {
		return $this->get_user_query()->where('users.slug', remove_special_characters($slug))->get('users')->row();
    }

    //get user by token
    public function get_user_by_token($token)
    {
		return $this->get_user_query()->where('users.token', remove_special_characters($token))->get('users')->row();
    }
    ...
    public function filter_users($role)
    {
        ...
        if (!empty($q)) {
            ...
            $this->db->or_like('nebim_code', clean_str($q));
            $this->db->group_end();
        }
