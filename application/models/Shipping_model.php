...
class Shipping_model extends CI_Model
{
    ...
    */
                ...
                if (!empty($seller)) {
                    ...
                    if (!empty($shipping_methods)) {
                        foreach ($shipping_methods as $shipping_method) {
                            ...
                            $method->free_shipping_min_amount = $free_shipping_min_amount = get_price($shipping_method->free_shipping_min_amount, "decimal");
                            $local_pickup_cost = get_price($shipping_method->local_pickup_cost, "decimal");
                            if ($shipping_method->method_type == "free_shipping") {
                                /* passive because seller total instead of cart total
                                 * isset($seller_total[$seller->id])
                                 * $seller_total[$seller->id]
                                 */
                                if (isset($cart_total->total_before_shipping)) {
                                    $total = $cart_total->total_before_shipping;
                            ...
                            $method->cart_total = null;
                            /* passive because seller total instead of cart total
                            if (isset($seller_total[$seller->id])) {
                                $method->cart_total = $seller_total[$seller->id];
                            }
                            */
                            $total = $total ?? 0;
                            if (isset($cart_total->total_before_shipping)) {
                                $method->cart_total = $cart_total->total_before_shipping;
                            }
                            if (!empty($method->cost) && $total <= $free_shipping_min_amount) {
                                $method->cost = number_format($method->cost, 2, ".", "");
                            }
                            //add shipping cost
                            $array_shipping_cost[$method->id] = $method->cost;
                            if ($set_session == true) {
                                $this->session->set_userdata('mc20bt99_array_shipping_cost', $array_shipping_cost);
                                $this->session->set_userdata('mc20bt99_array_cart_seller_ids', $seller_ids);
                            }
                            array_push($item->methods, $method);
                        }
                    }
                    // Min Tutar üstü ücretsiz gönderim
                    $filter_arr = array_filter($item->methods, function($method) {
                      return $method->is_free_shipping == 1;
                    });
                    $item->methods = $filter_arr ? $filter_arr:$item->methods;
                    // Min Tutar üstü ücretsiz gönderim
                ...
                if (!empty($item->methods)) {
                    ...
                    foreach ($item->methods as $method) {
                        if ($i == 0) {
                            ...
                            } elseif($total <= $free_shipping_min_amount) {
                                $method->is_selected = 1;
    ...
    public function calculate_cart_shipping_total_cost()
    {
        ...
        $array_shipping_cost = $this->session->userdata('mc20bt99_array_shipping_cost');
        $array_cart_seller_ids = $this->session->userdata('mc20bt99_array_cart_seller_ids');
        ...
        $this->session->set_userdata('mc20bt99_selected_shipping_method_ids', $selected_shipping_method_ids);
        $this->session->set_userdata('mc20bt99_seller_shipping_costs', $array_seller_shipping_costs);
        return $result;
    }
