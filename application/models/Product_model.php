...
class Product_model extends CI_Model
{
    ...
    public function build_query($type = "active", $compile_query = false)
    {
        $select = "products.*,
            ...
            (SELECT CONCAT(storage, '::', image_small) FROM images WHERE products.id = images.product_id ORDER BY id ASC LIMIT 1) AS image,
            (SELECT CONCAT(storage, '::', image_small) FROM images WHERE products.id = images.product_id ORDER BY id ASC LIMIT 1, 1) AS image_second,
            (SELECT GROUP_CONCAT(DISTINCT variation_options.option_names SEPARATOR '::') FROM variations INNER JOIN variation_options on variation_options.variation_id = variations.id WHERE variations.product_id = products.id and variations.nebim_option = 'Renk' AND variation_options.stock > 0 GROUP by variations.nebim_option) as variation_colors,
    ...
    public function filter_products($query_string_array = null, $category_ids = null, $custom_filters = null, $user_id = null)
    {
        ...
        if (!empty($sort) && $sort == "lowest_price") {
            ...
            $this->db->order_by('product_details.title', 'ASC');
        } elseif (!empty($sort) && $sort == "za") {
            $this->db->order_by('product_details.titlet', 'DESC');
        }*/ elseif (!empty($sort) && $sort == "most_popular") {
            $this->db->order_by('products.pageviews', 'DESC');
        } elseif (!empty($sort) && $sort == "rand") {
        ...
        if (!empty($categories)) {
            foreach ($categories as $category) {
                if ($category->show_subcategory_products == 1) {
                    ...
                    $this->db->group_end();
                    $this->db->order_by('products.is_promoted', 'DESC')->order_by('products.pageviews', 'DESC')->order_by('products.created_at', 'DESC')->limit($limit);
    ...
    {
        $wishlist = $this->session->userdata('mc20bt99_guest_wishlist');
    ...
    public function add_remove_wishlist($product_id)
    {
        if ($this->auth_check) {
            ...
            if ($this->is_product_in_wishlist($product_id)) {
                $wishlist = array();
                if (!empty($this->session->userdata('mc20bt99_guest_wishlist'))) {
                    $wishlist = $this->session->userdata('mc20bt99_guest_wishlist');
                }
                $new = array();
                if (!empty($wishlist)) {
                ...
                $this->session->set_userdata('mc20bt99_guest_wishlist', $new);
            } else {
                $wishlist = array();
                if (!empty($this->session->userdata('mc20bt99_guest_wishlist'))) {
                    $wishlist = $this->session->userdata('mc20bt99_guest_wishlist');
                ...
                $this->session->set_userdata('mc20bt99_guest_wishlist', $wishlist);
            }
        }
    }
