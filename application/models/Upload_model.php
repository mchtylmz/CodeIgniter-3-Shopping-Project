...
class Upload_model extends CI_Model
{
    ...
    public function product_default_image_upload($path, $folder, $filename = null)
    {
        if (!$filename) {
          $filename = generate_unique_id();
        }
        $new_name = 'img_x500_' . $filename . '_' . date('YmdHis') . '.' . pathinfo($path, PATHINFO_EXTENSION);
    ...
    public function product_big_image_upload($path, $folder, $filename = null)
    {
        if (!$filename) {
          $filename = generate_unique_id();
        }
        $new_name = 'img_1920x_' . $filename . '_' . date('YmdHis') . '.'. pathinfo($path, PATHINFO_EXTENSION);
        $new_path = 'uploads/' . $folder . '/' . $new_name;
        if ($folder == 'images') {
            $directory = $this->create_upload_directory('images');
            $new_name = $directory . $new_name;
    ...
    public function product_small_image_upload($path, $folder, $filename = null)
    {
        if (!$filename) {
          $filename = generate_unique_id();
        }
        $new_name = 'img_x300_' . $filename . '_' . date('YmdHis') . '.'. pathinfo($path, PATHINFO_EXTENSION);
        $new_path = 'uploads/' . $folder . '/' . $new_name;
        if ($folder == 'images') {
            $directory = $this->create_upload_directory('images');
            $new_name = $directory . $new_name;
    ...
    //story image upload
    public function story_image_upload($path)
    {
        $new_path = 'uploads/slider/story_' . generate_unique_id() . '.jpg';
        $img = Image::make($path)->orientate();
        $img->fit(900, 1600);
        $img->save(FCPATH . $new_path, $this->quality);
        return $new_path;
    ...
    //story avatar upload
    public function story_avatar_upload($path)
    {
        $new_path = 'uploads/slider/avatar_' . generate_unique_id() . '.jpg';
        $img = Image::make($path)->orientate();
        $img->fit(256, 256);
        $img->save(FCPATH . $new_path, $this->quality);
        return $new_path;
