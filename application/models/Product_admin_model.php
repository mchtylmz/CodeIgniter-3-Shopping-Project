...
class Product_admin_model extends CI_Model
{
    ...
    public function filter_products($list, $category_ids)
    {
        ...
        if (!empty($list)) {
            ...
            if ($list == "hidden_products") {
                ...
                $this->db->where('products.visibility', 0)->where('products.is_deleted', 0);
                $this->db->order_by('products.created_at', 'DESC');
            }
    ...
    //get paginated nostock product count
    public function get_paginated_nostock_products_count($list)
    {
        $category_ids = $this->get_filter_category_ids();
        $this->build_query();
        $this->filter_products($list, $category_ids);
    ...
    //get paginated nostock products
    public function get_paginated_nostock_products($per_page, $offset, $list)
    {
        $category_ids = $this->get_filter_category_ids();
        $this->build_query();
        $this->filter_products($list, $category_ids);
    ...
    //update_status_multi_products
    public function update_status_multi_products($product_ids, $status)
    {
