...
class Settings_model extends CI_Model
{
    ...
    public function update_general_settings()
    {
        $data = array(
            ...
            'onesignal_app_id' => $this->input->post('onesignal_app_id', false),
            'onesignal_key' => $this->input->post('onesignal_key', false),
            'onesignal_player_id' => $this->input->post('onesignal_player_id', false),
            'onesignal_test_mode' => $this->input->post('onesignal_test_mode', false)
        );
        $this->db->where('id', 1);
        return $this->db->update('general_settings', $data);
    }
    ...
    public function update_nebim_settings()
    {
        $data = array(
            'nebim_integrator_url' => $this->input->post('nebim_integrator_url', true),
            'nebim_office_code'    => $this->input->post('nebim_office_code', true),
            'nebim_store_code'     => $this->input->post('nebim_store_code', true),
            'nebim_warehouse_code' => $this->input->post('nebim_warehouse_code', true),
            'nebim_renk_customfield' => intval($this->input->post('nebim_renk_customfield', true)),
            'nebim_beden_customfield' => intval($this->input->post('nebim_beden_customfield', true)),
            'nebim_images_sync' => json_encode($this->input->post('nebim_images_sync', true))
        );
        $this->db->where('id', 1);
        return $this->db->update('general_settings', $data);
    ...
    public function update_payment_gateway($name_key)
    {
        ...
        if (!empty($gateway)) {
            $data = array(
                ...
                'nebim_v3' => trim($this->input->post('nebim_v3', true)),
                ...
                'commission' => $this->input->post('commission', true) == 1 ? 1 : 0,
                'installments' => json_encode(array_merge(["0"], $this->input->post('paynet_instalments', true))),
            );
    ...
    public function update_bank_transfer_settings()
    {
        $data = array(
            ...
            'bank_transfer_nebim_v3' => trim($this->input->post('bank_transfer_nebim_v3', true))
        );
        $this->db->where('id', 1);
    ...
    public function update_cash_on_delivery_settings()
    {
        $data = array(
            ...
            'cash_nebim_v3' => trim($this->input->post('cash_nebim_v3', true))
        );
        $this->db->where('id', 1);
        return $this->db->update('payment_settings', $data);
    }
    ...
    public function update_preferences($form)
    {
        if ($form == 'homepage') {
            ...
            $data = array(
                ...
                'nebim_status' => $this->input->post('nebim_status', true),
                'queue_status' => $this->input->post('queue_status', true) == '1' ? '1':'0',
                'home_story' => $this->input->post('home_story', true) == '1' ? '1':'0',
            );
    ...
    */
    ...
    public function get_selected_fonts()
    {
        ...
        $query = $this->db->query($sql, array(clean_number($this->settings->site_font), clean_number($this->settings->dashboard_font)))->row();
        return $query;
    }
    ...
    {
        return $this->db->query("SELECT * FROM fonts ORDER BY font_name")->result();
    }
    ...
    */
    ...
    {
        $this->db->where('id', 1);
        return $this->db->get('product_settings')->row();
    }
