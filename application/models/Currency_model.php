...
class Currency_model extends CI_Model
{
    ...
    public function set_selected_currency()
    {
        ...
        if (!empty($currency)) {
            $this->session->set_userdata('mc20bt99_selected_currency', $currency->code);
        }
    }
    ...
    {
        if ($this->payment_settings->currency_converter == 1 && !empty($this->session->userdata('mc20bt99_selected_currency'))) {
            if (isset($this->currencies[$this->session->userdata('mc20bt99_selected_currency')])) {
                return $this->currencies[$this->session->userdata('mc20bt99_selected_currency')];
            }
        }
    ...
    public function get_default_currency($currencies, $payment_settings)
    {
        ...
        $result = get_cached_data($this, "get_default_currency", "cr");
        if (!empty($result)) {
            return $result;
        }
        $currencies = $this->get_currencies();
        set_cache_data($this, "get_default_currency", $currencies[0] ?? '', "cr");
        return $currencies[0] ?? '';
    }
    ...
    {
        $result = get_cached_data($this, "get_currencies_array", "cr");
        if (!empty($result)) {
            return $result;
        }
        ...
        set_cache_data($this, "get_currencies_array", $array, "cr");
        return $array;
    }
