...
class Order_admin_model extends CI_Model
{
        ...
        if (!empty($order_products)) {
            ...
            foreach ($order_products as $order_product) {
                if ($order_product->order_status != "completed" && $order_product->order_status != "cancelled") {
                    $processing_count++;
                } elseif ($order_product->order_status == "completed") {
                    $completed_count++;
            ...
            if ($processing_count) {
                $data["status"] = 0;
            } elseif ($completed_count) {
                $data["status"] = 1;
            } elseif ($cancelled_count) {
                $data["status"] = 2;
            }
            $this->db->where('id', $order_id);
    ...
    public function filter_orders()
    {
        $data = array(
            ...
            'coupon' => $this->input->get('coupon', true)
        );
        ...
        if (!empty($data['user']) && $data['user']) {
            $this->db->where('orders.buyer_id', clean_number($data['user']));
        }
        if (!empty($data['coupon']) && $data['coupon']) {
            $this->db->where('orders.coupon_code', clean_str($data['coupon']));
        }
        if (!empty($data['order']) && $data['order']) {
            $this->db->where('orders.id', clean_number($data['order']));
        }
        $data['q'] = trim($data['q']);
        if (!empty($data['q'])) {
    ...
    */
    ...
    public function get_active_sales_count_by_seller($seller_id)
    {
        ...
        $this->db->where('order_products.seller_id', clean_number($seller_id))->where('order_status !=', 'completed')->where('order_status !=', 'cancelled');
        $this->db->distinct();
    ...
    //get cancelled sales count by seller
    public function get_cancelled_sales_count_by_seller($seller_id)
    {
        $this->db->select('orders.id');
        $this->db->join('order_products', 'order_products.order_id = orders.id');
        $this->db->where('order_products.seller_id', clean_number($seller_id))->where('order_status', 'cancelled');
        $this->db->distinct();
        return $this->db->count_all_results('orders');
    }
