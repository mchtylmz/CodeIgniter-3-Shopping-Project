...
class Order_model extends CI_Model
{
    ...
    {
        $cart_total = $this->session->userdata('mc20bt99_shopping_cart_total_final');
        if (!empty($cart_total)) {
            $data = array(
                'order_number' => uniqid(),
                'buyer_id' => 0,
                ...
                //'coupon_code' => $cart_total->coupon,
                //'coupon_discount' => get_price($cart_total->discount, "database"),
            ...
            if ($this->db->insert('orders', $data)) {
                ...
                // add queue
                $queue_id = add_queue('order_id', $order_id);
    ...
    {
        $cart_shipping = get_sess_data('mc20bt99_cart_shipping');
        if (!empty($cart_shipping)) {
            if ($cart_shipping->is_guest == true) {
                ...
                $state_city = !empty($billing_address['city']) ? get_city($billing_address['city']) : '';
                $data = array(
                    'order_id' => $order_id,
                    ...
                    'shipping_city' => !empty($state_city) ? $state_city->name : '',
                    ...
                    'billing_city' => !empty($state_city) ? $state_city->name : '',
                ...
                $country_shipping = !empty($shipping_address->country_id) ? get_country($shipping_address->country_id) : '';
                $state_shipping = !empty($shipping_address->state_id) ? get_state($shipping_address->state_id) : '';
                $city_shipping = !empty($shipping_address->city) ? get_city($shipping_address->city) : '';
                $country_billing = !empty($billing_address->country_id) ? get_country($billing_address->country_id) : '';
                $state_billing = !empty($billing_address->state_id) ? get_state($billing_address->state_id) : '';
                $state_city = !empty($billing_address->city) ? get_city($billing_address->city) : '';
                $data = array(
                    ...
                    'billing_city' => !empty($state_city) ? $state_city->name : '',
                    'billing_zip_code' => !empty($billing_address->zip_code) ? $billing_address->zip_code : '',
                    'biling_address_id' => clean_number($cart_shipping->billing_address_id ?? 0),
                    'biling_nebim_id' => !empty($billing_address->nebim_id) ? $billing_address->nebim_id : '',
                    'shipping_address_id' => clean_number($cart_shipping->shipping_address_id ?? 0),
                    'shipping_nebim_id' => !empty($shipping_address->nebim_id) ? $shipping_address->nebim_id : ''
                );
                if (active_nebimv3()) {
                  $data['biling_nebim_country'] = !empty($country_billing) ? $country_billing->nebim : '';
                  $data['biling_nebim_city'] = !empty($state_city) ? $state_city->nebim : '';
                  $data['biling_nebim_state'] = !empty($state_billing) ? $state_billing->nebim : '';
                  $data['shipping_nebim_country'] = !empty($country_shipping) ? $country_shipping->nebim : '';
                  $data['shipping_nebim_city'] = !empty($city_shipping) ? $city_shipping->nebim : '';
                  $data['shipping_nebim_state'] = !empty($state_shipping) ? $state_shipping->nebim : '';
                }
            }
        }
        if (!empty($data)) {
    ...
    {
        $order_id = clean_number($order_id);
        $cart_items = $this->session->userdata('mc20bt99_shopping_cart_final');
        $seller_shipping_costs = array();
        if (!empty($this->session->userdata('mc20bt99_seller_shipping_costs'))) {
            $seller_shipping_costs = $this->session->userdata('mc20bt99_seller_shipping_costs');
        }
        if (!empty($cart_items)) {
            foreach ($cart_items as $cart_item) {
                ...
                if (!empty($product)) {
                    ...
                    $data = array(
                        ...
                        'variation_option_barcodes' => $cart_item->barcode ?? null,
    ...
    public function add_payment_transaction($data_transaction, $order_id)
    {
        ...
        // Paynet
        try {
          $this->db->where('bank_code', $data_transaction["payment_id"]);
          $this->db->update('paynet_pos', ['order_id' => $order_id]);
        } catch (\Exception $e) {}
        // Paynet
    }
    ...
    {
        $this->db->where('id', clean_number($order_id));
        $query = $this->db->get('orders');
        return $query->row();
    }
            ...
            if ($order_product->seller_id == $this->auth_user->id) {
                ...
                if ($data["order_status"] == 'shipped') {
                    ...
                    if ($this->general_settings->send_email_order_shipped == 1) {
                        ...
                        $this->session->set_userdata('mc20bt99_send_email_data', json_encode($email_data));
                    }
                }
    ...
    public function get_completed_sales_count($user_id)
    {
        ...
        $this->db->where('order_products.seller_id', clean_number($user_id));
        $this->db->group_start();
        $this->db->or_where('order_products.order_status', 'completed');
        $this->db->or_where('order_products.order_status', 'cancelled');
        $this->db->group_end();
        $this->filter_sales();
        $query = $this->db->get('orders');
        return $query->num_rows();
        ...
        if (!empty($order_products)) {
            foreach ($order_products as $order_product) {
                ...
                if (!empty($product) && $product->product_type != "digital") {
                    ...
                    // ürün genel stok güncelleme
                    $product_model = product_model();
                    $product_model->update([
                      'stock' => intval($product_model->findOne($product->id)->variation_total_stock())
                    ], $product->id);
                    // ürün genel stok güncelleme
                }
            }
