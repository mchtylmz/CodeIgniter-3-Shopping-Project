...
class Cart_model extends CI_Model
{
    ...
    public function add_to_cart($product)
    {
        ...
        if ($update_quantity == 1) {
            $this->session->set_userdata('mc20bt99_shopping_cart', $cart);
        } else {
            $item = new stdClass();
            ...
            $item->find_barcode = end($options_array);
            $item->barcode = get_variation_option_barcode($item->find_barcode);
            $item->max_stock = get_variation_option_max_stock($item->find_barcode);
    ...
    public function remove_from_cart($cart_item_id)
    {
        ...
        if (!empty($cart)) {
            ...
            $this->session->set_userdata('mc20bt99_shopping_cart', $new_cart);
            if (count($new_cart) <= 0) {
              $this->clear_cart();
            }
        }
    ...
    public function get_product_price_and_stock($product, $cart_product_title, $options_array)
    {
        ...
        if (!empty($product)) {
            //quantity in cart
            $quantity_in_cart = 0;
            if (!empty($this->session->userdata('mc20bt99_shopping_cart'))) {
                foreach ($this->session->userdata('mc20bt99_shopping_cart') as $item) {
    ...
    public function get_sess_cart_items()
    {
        ...
        $this->cart_product_ids = array();
        if (!empty($this->session->userdata('mc20bt99_shopping_cart'))) {
            $cart = $this->session->userdata('mc20bt99_shopping_cart');
        }
        if (!empty($cart)) {
                ...
                if (!empty($product)) {
                    ...
                    if ($cart_item->purchase_type == 'bidding') {
                        ...
                        if (!empty($quote_request) && $quote_request->status == 'pending_payment') {
                            ...
                            $item->find_barcode = end($cart_item->options_array);
                            $item->barcode = get_variation_option_barcode($item->find_barcode);
                            $item->max_stock = get_variation_option_max_stock($item->find_barcode);
                            $item->product_sku = $product->sku;
    ...
    public function calculate_cart_total($cart_items, $currency_code = null, $set_session = true)
    {
        ...
        //set shipping cost
        if (!empty($this->session->userdata('mc20bt99_cart_shipping'))) {
            $shipping_cost = $this->session->userdata('mc20bt99_cart_shipping')->total_cost;
        ...
        // coupon
        $cart_total->coupon = session_coupon()->code ?? null;
        $cart_total->discount = coupon_discount_calculate($cart_total->subtotal);
        $cart_total->total = ($cart_total->subtotal - $cart_total->discount) + $cart_total->vat + $cart_total->shipping_cost;
        $cart_total->total_before_shipping = $cart_total->total_before_shipping - $cart_total->discount;
        // $cart_total->total_with_discount = $cart_total->total - $cart_total->discount;
        if (!empty($cart_items)) {
          if ($cart_total->total <= 0 || $cart_total->total_before_shipping <= 0) {
            session_coupon('delete');
            flashdata('coupon_error', trans('coupon_price_error'));
        ...
        // coupon
        if ($set_session == true) {
            $this->session->set_userdata('mc20bt99_shopping_cart_total', $cart_total);
        } else {
            return $cart_total;
        }
    }
    ...
    {
        $cart_total = new stdClass();
        if (!empty($this->session->userdata('mc20bt99_shopping_cart_total'))) {
            $cart_total = $this->session->userdata('mc20bt99_shopping_cart_total');
        }
        return $cart_total;
    }
    ...
    public function set_sess_cart_payment_method()
    {
        ...
        $this->session->set_userdata('mc20bt99_cart_payment_method', $std);
    }

    //get cart payment method option session
    public function get_sess_cart_payment_method()
    {
        if (!empty($this->session->userdata('mc20bt99_cart_payment_method'))) {
            return $this->session->userdata('mc20bt99_cart_payment_method');
        }
    }
    ...
    {
        if (!empty($this->session->userdata('mc20bt99_shopping_cart'))) {
            $this->session->unset_userdata('mc20bt99_shopping_cart');
        }
    }
    ...
    {
        if (!empty($this->session->userdata('mc20bt99_shopping_cart_total'))) {
            $this->session->unset_userdata('mc20bt99_shopping_cart_total');
        }
    }
    ...
    {
        if (!empty($this->session->userdata('mc20bt99_cart_payment_method'))) {
            $this->session->unset_userdata('mc20bt99_cart_payment_method');
        }
    }
    ...
    public function clear_cart()
    {
        ...
        if (!empty($this->session->userdata('mc20bt99_shopping_cart_final'))) {
            $this->session->unset_userdata('mc20bt99_shopping_cart_final');
        }
        if (!empty($this->session->userdata('mc20bt99_shopping_cart_total_final'))) {
            $this->session->unset_userdata('mc20bt99_shopping_cart_total_final');
        }
        if (!empty($this->session->userdata('mc20bt99_cart_shipping'))) {
            $this->session->unset_userdata('mc20bt99_cart_shipping');
        }
        session_coupon('delete');
        ...
        if (!empty($this->session->userdata('coupon_message'))) {
            $this->session->unset_userdata('coupon_message');
        }
		$this->remove_coupon();
    }
	...
	public function calculate_coupon_discount($cart_items)
	{
		...
		return ['total_discount' => $total_discount, 'seller_id' => $seller_id];
	}
