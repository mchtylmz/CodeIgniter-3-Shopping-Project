...
if (!empty($payment_gateway) && $payment_gateway->name_key == "mercado_pago"):
    ...
    try {
        ...
        $preference->back_urls = array(
            "success" => base_url() . "mercado-pago-payment-post?mc20bt99_lang=" . $this->selected_lang->short_form . "&mc20bt99_sess_id=" . $mc20bt99_payment_token,
            "failure" => base_url() . "mercado-pago-payment-post?mc20bt99_lang=" . $this->selected_lang->short_form . "&mc20bt99_sess_id=" . $mc20bt99_payment_token,
            "pending" => base_url() . "mercado-pago-payment-post?mc20bt99_lang=" . $this->selected_lang->short_form . "&mc20bt99_sess_id=" . $mc20bt99_payment_token
        );
        $preference->auto_return = "approved";
        //sale title
        $title = "";
        if ($mc20bt99_payment_type == 'membership') {
    ...
    <div id="payment-button-container" class="payment-button-cnt">
        ...
        <script src="https://www.mercadopago.com.uy/integrations/v1/web-payment-checkout.js" data-preference-id="<?php echo $preference->id; ?>" data-button-label="<?= trans("pay"); ?>&nbsp;<?= strip_tags(price_decimal($total_amount, $currency)); ?>"></script>
    </div>
<?php else: ?>
