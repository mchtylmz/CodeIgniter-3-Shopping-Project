            ...
            <div class="col-12">
                <div class="page-confirm">
                    ...
                    <div class="m-t-45 text-center">
                        <a href="<?php echo base_url(); ?>invoice-promotion/<?= $transaction->id; ?>" class="btn btn-lg btn-info color-white" target="_blank"><i class="icon-text-o"></i>&nbsp;&nbsp;<?php echo trans("view_invoice"); ?></a>
                        <a href="<?= generate_dash_url('products');?>" class="btn btn-lg btn-custom"><?= trans("go_back_to_products")?></a>
                    </div>
                </div>
            </div>
        </div>
...
if (!empty($this->session->userdata('mc20bt99_promote_bank_transaction_number'))) {
    $this->session->unset_userdata('mc20bt99_promote_bank_transaction_number');
}
if (!empty($this->session->userdata('mc20bt99_promoted_transaction_insert_id'))) {
    $this->session->unset_userdata('mc20bt99_promoted_transaction_insert_id');
}
?>
