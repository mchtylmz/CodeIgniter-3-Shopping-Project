...
<div class="box" id="infoo">
    <div class="box-header" style="background-color: #eee; padding-top: 10px; font-size: 16px;">
      Geçen Süre: <span id="uptime">00:00:00</span>
    </div>
    <div class="box-body p-1">
      <div style="display: flex; align-items:center;">
        <button id="tum_urunler" class="btn btn-primary m-2" type="button" onclick="downloadImages()" disabled>Resimleri Yükle</button>
...
$.ajax({
  url: "<?=base_url()?>ajax_controller/nebim_getproducts_images_count",
  type: 'POST',
  data: {},
  dataType: 'json',
  ...
  success: function(response) {
    nebim_getproducts_ids = response.products;
    text_tum_urunler.text(response.products.length + ' resimsiz ürün bulundu');
    btn_tum_urunler.removeAttr('disabled');
  },
...
async function startImport() {
  import_div.show();
  add_import_content('Resim aktarma işlemleri başladı, işlem tamamlanıncaya kadar sayfayı kapatmayınız!.', 'AKTARMA');
  import_progress.show();
  for (var index = 1; index <= nebim_getproducts_ids.length; index++) {
     window.variation_images = null;
     var now_index = (index - 1) * 9;
     var max_index = nebim_getproducts_ids.length * 9;
     progress_change(index, now_index, max_index);
     try {
       await get_product_image(nebim_getproducts_ids[index - 1].id);
     } catch (e) {
       add_import_content('SKU: ' + nebim_getproducts_ids[index - 1].sku + ' Bilinmeyen hata oluştu, resim aktarılamıyor', 'AKTARMA', 'red');
     }
  ...
  setTimeout(function() {
    add_import_content('Resim aktarma işlemleri tamamlandı!.', 'AKTARMA');
    Kronometre.bitir();
...
window.variation_images = null;
async function get_product_image(product_id) {
  return await $.ajax({
    url: "<?=base_url()?>ajax_controller/nebim_getproducts_image",
    type: 'POST',
    data: {
      id: product_id
    },
