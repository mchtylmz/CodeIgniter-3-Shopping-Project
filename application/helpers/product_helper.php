...
if (!function_exists('is_product_in_wishlist')) {
    function is_product_in_wishlist($product)
    {
        ...
        if ($ci->auth_check) {
            ...
            $wishlist = $ci->session->userdata('mc20bt99_guest_wishlist');
            if (!empty($wishlist)) {
...
if (!function_exists('get_variation_option_name')) {
    function get_variation_option_name($names_array, $lang_id)
    {
        ...
        if (!empty($names_array)) {
            ...
            if (empty($name) && is_array($names_array)) {
                foreach ($names_array as $item) {
                    if ($ci->general_settings->site_lang == $item['lang_id']) {
                        $name = $item['option_name'];
                        break;
...
//get parent variation option name
if (!function_exists('get_parent_variation_option_name')) {
    function get_parent_variation_option_name($parent_id, $lang_id)
    {
        $ci =& get_instance();
        $variation = $ci->variation_model->get_variation_option($parent_id);
        if ($variation) {
          return get_variation_option_name($variation->option_names, $lang_id);
        }
        return '';
    }
}

//get variation option barcode
if (!function_exists('get_variation_option_barcode')) {
    function get_variation_option_barcode($option_id)
    {
        $ci =& get_instance();
        $variation = $ci->variation_model->get_variation_option($option_id);
        if ($variation) {
...
//get variation option max stock
if (!function_exists('get_variation_option_max_stock')) {
    function get_variation_option_max_stock($option_id)
    {
        $ci =& get_instance();
        $variation = $ci->variation_model->get_variation_option($option_id);
        if ($variation) {
