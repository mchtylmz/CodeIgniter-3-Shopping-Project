...
$("form").submit(function () {
    $("input[name='" + mc20bt99_config.csfr_token_name + "']").val($.cookie(mc20bt99_config.csfr_cookie_name));
});

window.remove_cart_discount_coupon = function () {
	var data = {
		"sys_lang_id": mc20bt99_config.sys_lang_id
	};
	data[mc20bt99_config.csfr_token_name] = $.cookie(mc20bt99_config.csfr_cookie_name);
	$.ajax({
		url: mc20bt99_config.base_url + "ajax_controller/remove_cart_discount_coupon",
		type: "POST",
		data: data,
...
$(document).ready(function () {
    ...
    $('#main-slider').slick({
        ...
        infinite: true,
        speed: 500,
        fade: (mc20bt99_config.slider_fade_effect == 1) ? true : false,
        swipeToSlide: true,
        rtl: mc20bt99_config.rtl,
        cssEase: 'linear',
    ...
    $(document).ready(function () {
        baguetteBox.run('.product-slider', {
            animation: mc20bt99_config.rtl == true ? 'fadeIn' : 'slideIn'
        });
    });
...
function mobile_menu() {
    ...
    if (categories.length > 0) {
        ...
        if (obj_mobile_nav.back_button == 1) {
            ...
            document.getElementById("navbar_mobile_categories").innerHTML = '<li class="nav-item"><a href="' + item_all_link + '" class="nav-link">' + mc20bt99_config.txt_all + '</a></li>';
        }
...
$(document).ready(function () {
    $(".select2").select2({
        ...
        height: 42,
        dir: mc20bt99_config.rtl == true ? "rtl" : "ltr",
        "language": {
            "noResults": function () {
                return mc20bt99_config.txt_no_results_found;
            }
        },
    });
});
...
$(document).ready(function () {
    $("#form_login").submit(function (event) {
        ...
        if (form[0].checkValidity() === false) {
            ...
            serializedData.push({name: mc20bt99_config.csfr_token_name, value: $.cookie(mc20bt99_config.csfr_cookie_name)});
            serializedData.push({name: "sys_lang_id", value: mc20bt99_config.sys_lang_id});
            $.ajax({
                url: mc20bt99_config.base_url + "auth_controller/login_post",
                type: "post",
                data: serializedData,
                success: function (response) {
...
function send_activation_email(id, token) {
    ...
    $.ajax({
        type: "POST",
        url: mc20bt99_config.base_url + "auth_controller/send_activation_email_post",
        data: data,
        success: function (response) {
            var obj = JSON.parse(response);
            if (obj.result == 1) {
...
function select_product_variation_option(variation_id, variation_type, selected_option_id) {
    ...
    data[mc20bt99_config.csfr_token_name] = $.cookie(mc20bt99_config.csfr_cookie_name);
    var load_id = $('#variation_radio_button_' + variation_id).attr('data-loader');
    var beden_id = $('#variation_loader').attr('data-parent');
    if (variation_type == 'radio_button' && load_id && beden_id) {
      $('#variation_radio_button_' + beden_id).hide();
      $('#' + load_id).show();
      $(".btn-product-cart").attr("disabled", true);
    }
    $.ajax({
        type: "POST",
        url: mc20bt99_config.base_url + "select-variation-option-post",
        data: data,
        success: function (response) {
            var obj = JSON.parse(response);
            if (obj.status == 1) {
                ...
                if (obj.html_content_stock != "") {
                    ...
                    if (obj.stock_status == 0) {
                        ...
                        $(".btn-product-cart").attr("disabled", false);
								...
								if (obj.html_stock_total != "") {
									 $("input[name=product_quantity]").attr("data-max", obj.html_stock_total);
							  }
                if (obj.html_stock_total != "" && $("input[name=product_quantity]").val() > obj.html_stock_total) {
                   $("input[name=product_quantity]").val(obj.html_stock_total);
							  }
            ...
            if (variation_type == 'radio_button') {
                get_sub_variation_options(variation_id, selected_option_id, 'radio_button');
            }
        }
    });
}

function get_sub_variation_options(variation_id, selected_option_id, variation_type = 'dropdown') {
    var data = {
        "variation_id": variation_id,
        "selected_option_id": selected_option_id,
        "sys_lang_id": mc20bt99_config.sys_lang_id
    ...
    $.ajax({
        url: mc20bt99_config.base_url + "get-sub-variation-options",
        type: "POST",
        data: data,
        success: function (response) {
            var obj = JSON.parse(response);
            if (obj.status == 1) {
                ...
                if (variation_type == 'radio_button') {
                  if (selected_option_id == "") {
                      document.getElementById("variation_radio_button_" + obj.subvariation_id).innerHTML = "";
                  } else {
                      document.getElementById("variation_radio_button_" + obj.subvariation_id).innerHTML = obj.html_content;
                      $('#variation_radio_button_' + obj.subvariation_id).show();
                  }
                } // radio_button
                $(".btn-product-cart").attr("disabled", false);
            }
...
function update_number_spinner(btn) {
    var btn = btn,
        ...
        newVal = 0;
  let maxVal = btn.closest('.number-spinner').find('input').attr('data-max') ? $("input[name=product_quantity]").attr('data-max'):1;
    if (btn.attr('data-dir') == 'up') {
      newVal = parseInt(oldValue) + 1;
      if (newVal > parseInt(maxVal)) {
        newVal = maxVal;
      }
    } else {
        if (oldValue > 1) {
...
$(document).on("input paste change", ".cart-item-quantity .number-spinner input", function () {
    ...
    let dMax = parseInt($(this).attr('data-max')), dValue = parseInt($(this).val());
    if (dMax < dValue) {
      $(this).val(dMax);
      return false;
    }
    ...
    $.ajax({
        type: "POST",
        url: mc20bt99_config.base_url + "update-cart-product-quantity",
        data: data,
        success: function (response) {
            location.reload();
        }
...
function delete_review(review_id, product_id, user_id, message) {
    swal({
        ...
        if (willDelete) {
            ...
            $.ajax({
                method: "POST",
                url: mc20bt99_config.base_url + "home_controller/delete_review",
                data: data
            })
...
$(document).ready(function () {
    ...
    $("#form_add_comment").submit(function (event) {
        ...
        if (is_logged_in == false) {
            ...
            if (mc20bt99_config.is_recaptcha_enabled == true && is_logged_in == false) {
                if (g_recaptcha == "") {
        ...
        form_serialized.push({name: mc20bt99_config.csfr_token_name, value: $.cookie(mc20bt99_config.csfr_cookie_name)});
        form_serialized.push({name: "limit", value: parseInt($("#product_comment_limit").val())});
        form_serialized.push({name: "sys_lang_id", value: mc20bt99_config.sys_lang_id});

        $.ajax({
            url: mc20bt99_config.base_url + "ajax_controller/add_comment",
            type: "post",
    ...
    $("#form_add_subcomment_" + comment_id).ajaxSubmit({
        beforeSubmit: function () {
            ...
            if (is_logged_in == false) {
                ...
                if (mc20bt99_config.is_recaptcha_enabled == true) {
                    if (g_recaptcha == "") {
...
function load_more_comment(product_id) {
    ...
    $.ajax({
        url: mc20bt99_config.base_url + "ajax_controller/load_more_comment",
        type: "POST",
        data: data,
        success: function (response) {
            var obj = JSON.parse(response);
...
function delete_comment(comment_id, product_id, message) {
    swal({
        ...
        if (willDelete) {
            ...
            data[mc20bt99_config.csfr_token_name] = $.cookie(mc20bt99_config.csfr_cookie_name);
            $.ajax({
                url: mc20bt99_config.base_url + "ajax_controller/delete_comment",
                type: "POST",
                data: data,
                success: function (response) {
                    var obj = JSON.parse(response);
...
function show_comment_box(comment_id) {
    ...
    $.ajax({
        type: "POST",
        url: mc20bt99_config.base_url + "ajax_controller/load_subcomment_box",
        data: data,
        success: function (response) {
            var obj = JSON.parse(response);
            if (obj.type == 'form') {
...
$(document).ready(function () {
    ...
    $("#form_add_blog_comment").submit(function (event) {
        ...
        form_serialized.push({name: "sys_lang_id", value: mc20bt99_config.sys_lang_id});
        $.ajax({
            url: mc20bt99_config.base_url + "ajax_controller/add_blog_comment",
            type: "post",
            data: form_serialized,
            success: function (response) {
                if (mc20bt99_config.is_recaptcha_enabled == true && is_logged_in == false) {
...
function load_more_blog_comment(post_id) {
    ...
    $("#load_comment_spinner").show();
    $.ajax({
        url: mc20bt99_config.base_url + "ajax_controller/load_more_blog_comments",
        type: "post",
        data: data,
        success: function (response) {
            var obj = JSON.parse(response);
...
function delete_blog_comment(comment_id, post_id, message) {
    swal({
        ...
        if (willDelete) {
            ...
            data[mc20bt99_config.csfr_token_name] = $.cookie(mc20bt99_config.csfr_cookie_name);
            $.ajax({
                url: mc20bt99_config.base_url + "ajax_controller/delete_blog_comment",
                type: "post",
                data: data,
                success: function (response) {
                    var obj = JSON.parse(response);
...
function delete_conversation(conversation_id, message) {
    swal({
        ...
        if (willDelete) {
            ...
            $.ajax({
                method: "POST",
                url: mc20bt99_config.base_url + "message_controller/delete_conversation",
                data: data
            })
                .done(function (response) {
                    location.reload();
...
function remove_from_cart(cart_item_id) {
    ...
    $.ajax({
        type: "POST",
        url: mc20bt99_config.base_url + "cart_controller/remove_from_cart",
        data: data,
        success: function (response) {
            location.reload();
        }
...
$(document).on('click', '.btn-cart-product-quantity-item', function () {
    ...
    $.ajax({
        type: "POST",
        url: mc20bt99_config.base_url + "cart_controller/update_cart_product_quantity",
        data: data,
        success: function (response) {
            location.reload();
        }
...
function approve_order_product(id, message) {
    swal({
        ...
        if (approve) {
            ...
            $.ajax({
                type: "POST",
                url: mc20bt99_config.base_url + "order_controller/approve_order_product_post",
                data: data,
                success: function (response) {
                    location.reload();
                }
...
function get_shipping_methods_by_location(state_id) {
    ...
    $.ajax({
        type: "POST",
        url: mc20bt99_config.base_url + "cart_controller/get_shipping_methods_by_location",
        data: data,
        success: function (response) {
            var obj = JSON.parse(response);
            if (obj.result == 1) {
...
$(document).on("input paste click", "#input_location", function () {
    ...
    $.ajax({
        type: "POST",
        url: mc20bt99_config.base_url + "ajax_controller/search_location",
        data: data,
        success: function (response) {
            var obj = JSON.parse(response);
            if (obj.result == 1) {
...
$(document).on('click', '#btn_submit_location', function () {
    ...
    $.ajax({
        type: "POST",
        url: mc20bt99_config.base_url + "set-default-location-post",
        data: data,
        success: function (response) {
            location.reload();
        }
...
function report_abuse(form_id, item_type) {
    ...
    form_serialized.push({name: "sys_lang_id", value: mc20bt99_config.sys_lang_id});
    $.ajax({
        url: mc20bt99_config.base_url + "ajax_controller/report_abuse_post",
        type: "post",
        data: form_serialized,
        success: function (response) {
            var obj = JSON.parse(response);
 ...
 */
 ...
 function search_products(category, input_value, device) {
     ...
     var data = {
         "category": category,
         "input_value": input_value,
         "lang_base_url": mc20bt99_config.lang_base_url,
         "sys_lang_id": mc20bt99_config.sys_lang_id
     };
     data[mc20bt99_config.csfr_token_name] = $.cookie(mc20bt99_config.csfr_cookie_name);
     $.ajax({
         type: "POST",
         url: mc20bt99_config.base_url + "ajax_controller/ajax_search",
         data: data,
         success: function (response) {
             var obj = JSON.parse(response);
             if (obj.result == 1) {
...
$(document).on("change", "#select_sort_items", function () {
    ...
    var current_url = $(this).attr('data-current-url');
    var page = $(this).attr('data-page');
    if (val == "most_recent" || val == "lowest_price" || val == "highest_price" || val == "most_popular" || val == "az" | val == "za") {
        var params = "sort=" + val;
        if (query_string == "") {
            query_string = "?" + params;
        } else {
...
function set_site_language(lang_id) {
    ...
    $.ajax({
        method: "POST",
        url: mc20bt99_config.base_url + "home_controller/set_site_language",
        data: data
    })
        .done(function (response) {
            location.reload();
...
function load_more_promoted_products() {
    ...
    $.ajax({
        type: "POST",
        url: mc20bt99_config.base_url + "home_controller/load_more_promoted_products",
        data: data,
        success: function (response) {
            var obj = JSON.parse(response);
            if (obj.result == 1) {
...
$("#form_send_message").submit(function (event) {
    ...
    $.ajax({
        url: mc20bt99_config.base_url + "message_controller/add_conversation",
        type: "post",
        data: serializedData,
        success: function (response) {
...
function send_message_as_email(message_sender_id, message_receiver_id, message_subject, message_text) {
    ...
    $.ajax({
        type: "POST",
        url: mc20bt99_config.base_url + "ajax_controller/send_email",
        data: data,
        success: function (response) {
        }
    });
...
function get_states(val, map, id_suffix = "") {
    ...
    $.ajax({
        type: "POST",
        url: mc20bt99_config.base_url + "ajax_controller/get_states",
        data: data,
        success: function (response) {
            var obj = JSON.parse(response);
            if (obj.result == 1) {
...
function get_cities(val, map, id_suffix = "") {
    var data = {
        "state_id": val,
        "sys_lang_id": mc20bt99_config.sys_lang_id
    };
    ...
    $.ajax({
        type: "POST",
        url: mc20bt99_config.base_url + "ajax_controller/get_cities",
        data: data,
        success: function (response) {
            var obj = JSON.parse(response);
            if (obj.result == 1) {
                document.getElementById("select_cities" + id_suffix).innerHTML = obj.content;
                $('#get_cities_container' + id_suffix).show();
            } else {
                document.getElementById("select_cities" + id_suffix).innerHTML = "";
                $('#get_cities_container' + id_suffix).hide();
            }
            if (map) {
                update_product_map();
...
$(document).on('click', '.btn-add-remove-wishlist', function () {
    ...
    if (data_type == "details") {
        if ($(this).find("i").hasClass("icon-heart-o")) {
            $('.btn-add-remove-wishlist').html('<i class="icon-heart"></i><span>' + mc20bt99_config.txt_remove_from_wishlist + '</span>');
        } else {
            $('.btn-add-remove-wishlist').html('<i class="icon-heart-o"></i><span>' + mc20bt99_config.txt_add_to_wishlist + '</span>');
        }
    }
    var data = {
        "product_id": product_id,
    ...
    $.ajax({
        type: "POST",
        url: mc20bt99_config.base_url + "add-remove-wishlist-post",
        data: data,
        success: function (response) {
        }
    });
...
$(document).on('click', '.btn-item-add-to-cart', function () {
    ...
    $.ajax({
        type: "POST",
        url: mc20bt99_config.base_url + "cart_controller/add_to_cart",
        data: data,
        success: function (response) {
          var obj = JSON.parse(response);
          if (obj.result == 1) {
            ...
            /*
            setTimeout(function () {
                window.location.href = mc20bt99_config.lang_base_url + mc20bt99_config.cart_route;
            }, 100);
            */
        }
    });
...
function hide_cookies_warning() {
    ...
    $.ajax({
        type: "POST",
        url: mc20bt99_config.base_url + "home_controller/cookies_warning",
        data: data,
        success: function (response) {
        }
    });
...
$("#form_add_cart").submit(function (event) {
    ...
    $('#form_add_cart .custom-control-variation input').each(function () {
        ...
        // warning no stock
        var selected_inputs = $( "#form_add_cart .custom-control-variation input:checked" );
        var out_of_stock = $(this).attr('data-out-of-stock');
        for (var input = 0; input < selected_inputs.length; input++) {
          if (selected_inputs.eq(input).val() == out_of_stock) {
            disabled = true;
          }
        } //  for
    });
    if (disabled) {
      try {
        var message = $choose_option_no_stock;
      } catch (e) {
        var message = 'Stok olmayan ürün eklenemez!';
      }
      swal({
          text: message,
          icon: "warning",
    ...
    if (add_to_cart) {
        ...
        serializedData.push({name: "sys_lang_id", value: mc20bt99_config.sys_lang_id});
        $.ajax({
            url: mc20bt99_config.base_url + "add-to-cart",
            type: "post",
            data: serializedData,
            success: function (response) {
                var obj = JSON.parse(response);
                if (obj.result == 1) {
                    setTimeout(function () {
                        $('#form_add_cart .btn-product-cart').html('<i class="icon-check"></i>' + mc20bt99_config.txt_added_to_cart);
                        $('.span_cart_product_count').html(obj.product_count);
                        $('.span_cart_product_count').removeClass('visibility-hidden');
                        $('.span_cart_product_count').addClass('visibility-visible');
                    }, 100);
                    setTimeout(function () {
                        $('#form_add_cart .btn-product-cart').html('<span class="btn-cart-icon"><i class="icon-cart-solid"></i></span>' + mc20bt99_config.txt_add_to_cart);
...
$(document).ready(function () {
    $('.validate_terms').submit(function (e) {
        ...
        if (!$('.custom-control-validate-input input').is(":checked")) {
            ...
            $('.custom-control-validate-input').append("<p class='text-danger'>" + mc20bt99_config.msg_accept_terms + "</p>");
        } else {
            $('.custom-control-validate-input').removeClass('custom-control-validate-error');
        }
    });
...
$(document).on("input keyup paste change keypress", ".price-input", function () {
    if (typeof mc20bt99_config.thousands_separator == 'undefined') {
        mc20bt99_config.thousands_separator = '.';
    }
    if (mc20bt99_config.thousands_separator == '.') {
        var $this = $(this);
...
function delete_quote_request(id, message) {
    swal({
        ...
        if (willDelete) {
            ...
            $.ajax({
                type: "POST",
                url: mc20bt99_config.base_url + "bidding_controller/delete_quote_request",
                data: data,
                success: function (response) {
                    location.reload();
                }
...
function get_product_shipping_cost(val, product_id) {
    ...
    $.ajax({
        type: "POST",
        url: mc20bt99_config.base_url + "ajax_controller/get_product_shipping_cost",
        data: data,
        success: function (response) {
            var obj = JSON.parse(response);
            if (obj.result == 1) {
...
function delete_shipping_address(id, message) {
    swal({
        ...
        if (willDelete) {
            ...
            $.ajax({
                type: "POST",
                url: mc20bt99_config.base_url + "profile_controller/delete_shipping_address_post",
                data: data,
                success: function (response) {
                    location.reload();
                }
